...
namespace LoginSystemPowerCode.Systems
{
    public class MgtDatabase
    {
        ...
        public Juego ObtenerJuegoPorId(int juegoId)
        {
            // Se obtiene la lista completa de juegos
            List<Juego> juegos = ObtenerTodosLosJuegos();
            // Se retorna el primer juego que coincida con el Id
            return juegos.FirstOrDefault(j => j.Id == juegoId);
        }
        ...
        public string ObtenerImagenPorId(int juegoId)
        {
            ...
            using (SQLiteConnection connection = new SQLiteConnection(sacarConnection()))
            {
                ...
                string query = "SELECT imagen FROM imagenes WHERE juego_id = @juegoId";
                using (SQLiteCommand command = new SQLiteCommand(query, connection))
                {
